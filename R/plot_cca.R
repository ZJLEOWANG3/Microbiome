...
plot_cca <- function(physeq, grouping_column, pvalueCutoff=0.01,norm_method=NULL, env.variables=NULL,
  ...
  var = colnames(meta_table)
    ...
    for (i in seq_along(var)){
      levelvari = meta_table[, var[i]]
      if (length(unique(levelvari)) >= 2) {
      abund_table.adonis <- vegan::adonis2(as.formula(paste("abund_table","~",
                            var[i],sep=" ")), data = meta_table)
      if (abund_table.adonis$"Pr(>F)"[1]<=pvalueCutoff) 
        bestEnvVariables <- c(bestEnvVariables,var[i])
    }}
