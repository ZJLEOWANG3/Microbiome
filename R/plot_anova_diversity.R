...
plot_anova_diversity <- function(physeq,method, grouping_column,pValueCutoff=0.05, outfile="anova_diversity.csv")
{
  ...
  if(!is.null(df_pw)){ #this only happens when we have significant pairwise anova results
    for(i in 1:dim(df_pw)[1]){
      leveldfi = levels(df[, grouping_column])
      xfrom = which(leveldfi == c(df_pw[i, "from"]))
      xto = which(leveldfi == c(df_pw[i, "to"]))
      yheight = as.numeric(as.character(df_pw[i,  "y"]))
      measure = as.character(df_pw[i, "measure"])
      pvalue = as.numeric(df_pw[i, "p"])
      pvector = c(paste("p =", pvalue, ", ***"), 
                  paste("p =", pvalue, ", **"),
                  paste("p =", pvalue, ", *"), 
                  paste("p =", pvalue))
      
      p <- p + geom_path(inherit.aes = F, aes(x, y), 
                         data = data.frame(x = c(xfrom, xto), 
                                           y = c(yheight, 
                                                 yheight), 
      ...
      p <- p + geom_text(inherit.aes = F, aes(x = x, y = y, label = label),
                         data = data.frame(x = (xfrom + xto)/2,
                                           y = yheight * 1.1,
                                           measure = measure,
                                           label = as.character(cut(pvalue,
